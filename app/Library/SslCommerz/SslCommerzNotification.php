...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    /**
     * Get the SSLCommerz validation data
     * @return object|null
     */
    public function getValidationData()
    {
        return $this->sslc_data ?? null;
    }
    ...
    protected function setSuccessUrl()
    {
        ...
        $this->successUrl = (strpos($url, 'http://') === 0 || strpos($url, 'https://') === 0)
            ? $url
            : rtrim(env('APP_URL'), '/') . $url;
    }
    ...
    protected function setFailedUrl()
    {
        ...
        $this->failedUrl = (strpos($url, 'http://') === 0 || strpos($url, 'https://') === 0)
            ? $url
            : rtrim(env('APP_URL'), '/') . $url;
    }
    ...
    protected function setCancelUrl()
    {
        ...
        $this->cancelUrl = (strpos($url, 'http://') === 0 || strpos($url, 'https://') === 0)
            ? $url
            : rtrim(env('APP_URL'), '/') . $url;
    }
    ...
    protected function setIPNUrl()
    {
        ...
        $this->ipnUrl = (strpos($url, 'http://') === 0 || strpos($url, 'https://') === 0)
            ? $url
            : rtrim(env('APP_URL'), '/') . $url;
    }
